...
-(void)launchSelectedItemViewController:(UIViewController*)itemViewController withTopTitle:(NSString*)title {
    ...
    if (![[UIApplication sharedApplication] isStatusBarHidden]) {
        [self.launcherNavigationController.view setTop:-20];
    }
    
    
    UIButton *leftButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [leftButton setFrame:CGRectMake(0, 0, 20, 20)];
    [leftButton setImage:[UIImage imageNamed:@"dismiss"] forState:UIControlStateNormal];
    [leftButton addTarget:self action:@selector(closeView) forControlEvents:UIControlEventTouchUpInside];
    
	[itemViewController.navigationItem setLeftBarButtonItem:[[UIBarButtonItem alloc] initWithCustomView:leftButton]];
...
-(void)launcherViewDidBeginEditing:(id)sender {
    ...
    [rightButton setImage:[UIImage imageNamed:@"profileCheckmark"] forState:UIControlStateNormal];
    [rightButton addTarget:self.launcherView action:@selector(endEditing) forControlEvents:UIControlEventTouchUpInside];
