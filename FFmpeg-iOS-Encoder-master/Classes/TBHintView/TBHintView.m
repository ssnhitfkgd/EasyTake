...
-(UIButton *)buttonDismiss
{
    if( !buttonDismiss )
    {
        ...
        [buttonDismiss setImage:[UIImage imageNamed:@"close"] forState:UIControlStateNormal];
...
- (void)showInputTextField
{
    ...
    UIImage *inputImage = [[UIImage imageNamed:@"loginInput"] stretchableImageWithLeftCapWidth:10 topCapHeight:0];
    UIImageView *vInputImageView = [[[UIImageView alloc] initWithFrame:CGRectMake(22, 70, 260, 92)] autorelease];
    [vInputImageView setTag:1];
    [vInputImageView setImage:inputImage];
    [self addSubview:vInputImageView];
    
    UITextField *userText = [[[UITextField alloc] initWithFrame:CGRectMake(vInputImageView.left + 10, vInputImageView.top + 7, 240, 40)] autorelease];
    ...
    userText.placeholder = @"邮箱";
    [userText becomeFirstResponder];
    self.userName = userText;
    [userText setTextColor:[UIColor colorWithRed:40.0/255.0 green:40.0/255.0  blue:40.0/255.0  alpha:1.0]];
    [self addSubview:userText];
    [self bringSubviewToFront:userText];
    
    UITextField *pwdText = [[[UITextField alloc] initWithFrame:CGRectMake(userText.left, userText.bottom + 4, 240, 40)] autorelease];
    [pwdText setBackgroundColor:[UIColor clearColor]];
    pwdText.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    pwdText.autocapitalizationType = UITextAutocapitalizationTypeNone;
    ...
    [pwdText setTextColor:[UIColor colorWithRed:40.0/255.0 green:40.0/255.0  blue:40.0/255.0  alpha:1.0 ]];
    [self addSubview:pwdText];
    [self bringSubviewToFront:pwdText];
    userText.keyboardType = UIKeyboardTypeEmailAddress;
